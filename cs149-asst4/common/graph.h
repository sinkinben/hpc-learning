...
/* 1) Each edge is represented by an integer describing the id of the destination vertex.
 * 2) Edges are stored in the graph sorted by their source vertex, so the source vertex 
...
struct graph
{
     ...
     * 
     * 1）outgoing_starts[i] it denotes the offset in outgoing_starts of vertex `i`.
     * 2) (i, outgoing_edges[outgoing_starts[i]]) denotes the first out-edge of `i`.
     * 3) (i, outgoing_edges[outgoing_starts[i + 1] - 1]) denotes the last out-edge of `i`.
     */
    int *outgoing_starts;
    Vertex *outgoing_edges;

    /* (incoming_edges[incoming_start[i]], i) denote the first in-edge of `i`
     * (incoming_edges[incoming_start[i + 1] - 1], i) denote the last in-edge of `i`
     */
    int *incoming_starts;
